...
class HBSNet(BaseNet):
    def __init__(self, config: HBSNetConfig, backbone: UNet, pre_stn: Optional[STN]=None, post_stn: Optional[STN]=None):
    ...
    def create_mask(self, r):
        x, y = torch.meshgrid(torch.arange(self.config.output_width),torch.arange(self.config.output_height), indexing='ij')
    ...
    def loss(self, predict_hbs, ground_truth_hbs, is_mask=True):
        ...
        output_data = (predict_hbs, ground_truth_hbs)
        ...
        if is_mask:
            predict_hbs = torch.masked_select(predict_hbs, self.mask)
            ground_truth_hbs = torch.masked_select(ground_truth_hbs, self.mask)
        ...
        loss = hbs_loss + self.config.stn_rate * stn_loss + self.config.grad_rate * grad_loss
        loss_dict = {
            "loss": loss,
            "hbs_loss": hbs_loss,
            "stn_loss": stn_loss,
    ...
    def factory(cls, config: HBSNetConfig):
        backbone = UNet(
            n_channels=config.input_channels, n_classes=config.output_channels,
            channels_down=config.channels_down, channels_up=config.channels_up,
            is_bilinear=True, dtype=config.dtype, is_skip=config.is_skip
        )
        
        pre_stn = STN(
            input_channels=config.input_channels, height=config.height, width=config.width,
            dtype=config.dtype, stn_mode=1
        ) if config.stn_mode in [1, 3] else None
        
        post_stn = STN(
            input_channels=config.output_channels, height=config.output_height, width=config.output_width,
            dtype=config.dtype, stn_mode=2
        ) if config.stn_mode in [2, 3] else None
