...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True, dtype=DTYPE):
        super().__init__()

        # if bilinear, use the normal convolutions to reduce the number of channels
        if bilinear:
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.conv = DoubleConv(in_channels, out_channels, in_channels // 2, dtype=dtype)
        else:
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=False, dtype=DTYPE):
        super(UNet, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear

        self.inc = (DoubleConv(n_channels, 64, dtype=dtype))
        self.down1 = (Down(64, 128, dtype=dtype))
        self.down2 = (Down(128, 256, dtype=dtype))
        self.down3 = (Down(256, 512, dtype=dtype))
        factor = 2 if bilinear else 1
        self.down4 = (Down(512, 1024 // factor, dtype=dtype))
        self.up1 = (Up(1024, 512 // factor, bilinear, dtype=dtype))
        self.up2 = (Up(512, 256 // factor, bilinear, dtype=dtype))
        self.up3 = (Up(256, 128 // factor, bilinear, dtype=dtype))
        self.up4 = (Up(128, 64, bilinear, dtype=dtype))
        self.outc = (OutConv(64, n_classes, dtype=dtype))
