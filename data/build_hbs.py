...
def start_eng(shared_name="hbs_engine"):
    eng = matlab.engine.start_matlab(
        f"-nojvm -nodisplay -nosplash -nodesktop -r \"matlab.engine.shareEngine('{shared_name}')\""
    )
    init_setting(eng)
...
@click.option(
    "--image_dir",
    default=f"{ROOT_DIR}/img/generated",
    help="Directory of images to generate HBS",
)
@click.option(
    "--eng_name",
    default="hbs_engine",
    help="Shared name of the matlab engine",
)
...
@click.option(
    ...
    "--is_random",
    is_flag=True,
    help="Randomize the order of the images",
)
def main(image_dir, eng_name, unit_disk_radius, is_random):
    ...
    if is_random:
        file_list = random.sample(
            os.listdir(image_dir), len(os.listdir(image_dir))
        )
    else:
        ...
        try:
            if file.endswith(".png"):
                ...
                try:
                    ...
                    logger.error(
                        f"Timeout, failed to generate and removed image {image_path}"
                    )
